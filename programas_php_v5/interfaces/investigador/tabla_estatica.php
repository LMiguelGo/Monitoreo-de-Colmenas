...
if ($_SESSION["autenticado"] != "SIx3") {
    header("Location: ../inicio_sesion/index.php?mensaje=3"); //OK
    exit;
} else {
    if ($_SESSION["tipo_usuario"] != "investigador")
        header('Location: ../inicio_sesion/index.php?mensaje=4'); //OK
}
...
if ($id_colmena > 0) {
    ...
    if (!empty($fecha_inicio) && !empty($fecha_fin)) {
        $sql1 .= " AND d.fecha BETWEEN '$fecha_inicio' AND '$fecha_fin'";
    }

    if (!empty($hora_inicio) && !empty($hora_fin)) {
        $sql1 .= " AND d.hora BETWEEN '$hora_inicio' AND '$hora_fin'";
    }

    if (!empty($fecha_inicio) && !empty($fecha_fin) && !empty($hora_inicio) && !empty($hora_fin)) {
        $sql1 .= " OR (CONCAT(d.fecha, ' ', d.hora) BETWEEN '$fecha_inicio $hora_inicio' AND '$fecha_fin $hora_fin')";
    }

    $sql1 .= " ORDER BY d.fecha DESC, d.hora";
...
<head>
    ...
    <style>
      ...
      /* Subtítulo visual igual al título */
      .titulo {
        margin-left: 00px;
        color: #cc6600;
        font-size: 16px;
      ...
      select, input[type="submit"], input[type="date"], input[type="time"], button {
        padding: 8px 12px;
...
<body>
    <header>
      ...
      <div class="user-info">
        ...
        <a href="../inicio_sesion/includes/cerrar_sesion.php" class="btn">Cerrar Sesión</a>  <!-- OK -->
      </div>
    </header>

    <?php include "includes/cinta_investigador.php"; ?> <!-- OK -->

    <div class="container">
        <h2><?php echo $titulo; ?></h2>

        <form method="GET" action="tabla_estatica.php">
            ...
            <p class="titulo">Rango de consulta</p>
            ...
            <button type="button" onclick="limpiarRango()">Limpiar</button>
            <br><br>
        ...
        <?php } elseif ($id_colmena > 0) { ?>
            <p style="text-align:center; color:red;">No hay datos registrados para esta colmena en el rango seleccionado.</p>
        <?php } ?>
    </div>
